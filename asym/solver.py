...
class Solver(object):
    ...
    def train(self):
        ...
        for epoch in range(self.n_epochs):
            ...
            if score > best_loss:
                print('best loss!')
                best_loss = score
                torch.save(self.model.state_dict(), os.path.join(self.model_save_path, 'best_loss.pth'))
                torch.save(self.model.state_dict(), os.path.join(self.model_save_path, 'best_model.pth'))
            if roc_auc > best_roc:
                print('best roc!')
                best_roc = roc_auc
                torch.save(self.model.state_dict(), os.path.join(self.model_save_path, 'best_roc.pth'))
                torch.save(self.model.state_dict(), os.path.join(self.model_save_path, 'best_model.pth'))
            if pr_auc > best_pr:
                print('best pr!')
                best_pr = pr_auc
                torch.save(self.model.state_dict(), os.path.join(self.model_save_path, 'best_pr.pth'))
                torch.save(self.model.state_dict(), os.path.join(self.model_save_path, 'best_model.pth'))
    ...
    def opt_schedule(self, current_optimizer, drop_counter):
        ...
        if current_optimizer == 'adam' and drop_counter == 60:
            self.load(os.path.join(self.model_save_path, 'best_loss.pth'))
            self.optimizer = torch.optim.SGD(self.model.parameters(), 0.001, momentum=0.9, weight_decay=0.0001, nesterov=True)
            current_optimizer = 'sgd_1'
            drop_counter = 0
            print('sgd 1e-3')
    ...
    def get_tensor(self, fn):
        # load audio
        if self.dataset == 'mtat':
            raw_path = os.path.join(self.data_path, 'npy', fn.split('/')[1][:-3]) + 'npy'
