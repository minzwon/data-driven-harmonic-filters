...
def initialize_filterbank(sample_rate, n_harmonic, semitone_scale):
    ...
    for i in range(n_harmonic):
        _hhz = midi_to_hz(midi + 12*i)
        _gap = np.diff(_hhz)
        harmonic_hz = np.concatenate((harmonic_hz, _hhz[1:-1]))
        lower_bw = np.concatenate((lower_bw, _gap[:-1]))
        upper_bw = np.concatenate((upper_bw, _gap[1:]))
...
class Conv_init(nn.Module):
    def __init__(self, input_channels, output_channels, pooling=2):
        ...
        self.mp = nn.MaxPool2d(pooling)
    def forward(self, x):
        f0_out = self.relu(self.bn_spec(self.conv_spec(x[:,0].unsqueeze(1)))) # melspec conv
        harmonic_out = self.relu(self.bn_H(self.conv_H(x.unsqueeze(1)))).squeeze(2) # harmonic conv
