...
class HarmonicConv(nn.Module):
    def __init__(self,
        ...
        self.lower_bw = torch.tensor(lower_bw.astype('float32'))
        self.upper_bw = torch.tensor(upper_bw.astype('float32'))
        self.fft_bins = torch.linspace(0, self.sample_rate//2, 257)
...
class ResNet_mp(nn.Module):
    def __init__(self, input_channels, conv_channels):
        ...
        self.res2 = Conv3_2d(conv_channels, conv_channels, 2)
        self.res3 = Conv3_2d(conv_channels, conv_channels*2, 2)
        self.res4 = Conv3_2d(conv_channels*2, conv_channels*2, 2)
        self.res5 = Conv3_2d(conv_channels*2, conv_channels*2, (2,3))
        self.res6 = Conv3_2d(conv_channels*2, conv_channels*2, (2,3))
        self.res7 = Conv3_2d(conv_channels*2, conv_channels*2, (2,3))
...
class Conv_init(nn.Module):
    def __init__(self, input_channels, output_channels, pooling=2):
        ...
        self.conv_1 = nn.Conv2d(1, output_channels, 3, padding=1)
        self.bn_1 = nn.BatchNorm2d(output_channels)
        self.conv_2 = nn.Conv2d(input_channels, output_channels, 3, padding=1)
        self.bn_2 = nn.BatchNorm2d(output_channels)
        self.conv_3 = nn.Conv2d(output_channels, output_channels, 3, padding=1)
        self.bn_3 = nn.BatchNorm2d(output_channels)
        self.relu = nn.ReLU()
        self.mp = nn.MaxPool2d(pooling)
    def forward(self, x):
        f0_out = self.relu(self.bn_1(self.conv_1(x[:,0].unsqueeze(1))))
        harmonic_out = self.relu(self.bn_2(self.conv_2(x)))
        out = f0_out + harmonic_out
        out = self.mp(self.relu(self.bn_3(self.conv_3(out))))
        return out
